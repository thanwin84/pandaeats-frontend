    ...
    <Card>
      <CardHeader>
        <CardTitle className="grid grid-cols-1 gap-2 md:grid-cols-4 pb-1">
          ...
          <span>
            Time:{' '}
            <span className="font-normal">{time.toLocaleTimeString()}</span>
          </span>
          <span>
            Total Cost: <span className="font-normal">{order.totalAmount}</span>
          </span>
        </CardTitle>
        <Separator />
        <CardDescription>Order number: {orderNumber + 1}</CardDescription>

        <CardContent className="p-0 space-y-2">
          <p className="font-medium">Ordered Items list:</p>
          <ul className="pl-2">
            {order.cartItems?.map((cartItem, index) => (
              <li className="flex gap-2 items-center mb-1" key={cartItem._id}>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              <SelectTrigger
                ...
                className="w-full border rounded-md text-start pl-3 py-1"
              >
