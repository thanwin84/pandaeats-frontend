...
export default function OrderCard({ className, order }: Props) {
  const time = new Date();
  ...
  return (
    <div
      className={`px-4 py-4 bg-gray-50 border shadow-sm rounded-md  ${className}`}
    >
      <OrderStatusHeader status={order.status} time={expactedTime} />
